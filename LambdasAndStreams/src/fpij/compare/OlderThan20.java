...
public class OlderThan20 {
    ...
    static void test() {
        ...
        System.out.println("p " + olderThan20);

        List<Person> lst = people.stream()
            .filter(p -> p.getAge() > 20)
            .collect(Collectors.toList());

        System.out.println("P " + lst);


        Map<Integer, List<Person>> pplAge = people.stream()
            .collect(Collectors.groupingBy(Person::getAge));

        System.out.println("G" + pplAge);
    }
    /**
     * Group by Age: {35=[}
     */
